...
echo -e "${yellow}Checking npm installation...${neutral}"
if ! command -v npm &> /dev/null; then
    echo -e "${yellow}npm not found. Installing npm...${neutral}"
    apt-get update
    apt-get install -y npm
    echo -e "${green}✅ npm installed successfully${neutral}"
else
    echo -e "${green}✅ npm is already installed${neutral}"
fi
...
if [ ! -d "node_modules" ]; then
    ...
    echo -e "${yellow}Adding axios to existing installation...${neutral}"
    npm install axios
    echo -e "${green}✅ Axios installed successfully${neutral}"
fi
