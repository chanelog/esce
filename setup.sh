...
echo "   🔍 Mengecek izin VPS..."
echo "   IP VPS: ${MYIP}"
sleep 1

# ============ CEK IP DENGAN CURL (PEYXDEV REPO) ============
echo "   📡 Cek repo lisensi..."

# Cek repo utama
CEK_REPO_IP=$(curl -s --max-time 5 https://raw.githubusercontent.com/peyxdev/esce/main/ip 2>/dev/null | grep "$MYIP")
CEK_REPO_IPX=$(curl -s --max-time 5 https://raw.githubusercontent.com/peyxdev/esce/main/ipx 2>/dev/null | grep "$MYIP")

if [[ -n "$CEK_REPO_IP" || -n "$CEK_REPO_IPX" ]]; then
    echo "   ✅ IP terdaftar"
    SKIP_REG=true
else
    echo "   ❌ IP tidak ditemukan di repo"
    SKIP_REG=false
fi
...
if [[ "$SKIP_REG" == true ]]; then
    echo "   ⏭️ Skip registrasi (IP sudah aktif)"
else
    echo "   ⚠️ IP Anda belum terdaftar."
    echo "   Hubungi Admin PeyxDev untuk aktivasi."
    # Jika Anda ingin tetap melanjutkan proses meskipun belum terdaftar:
    # sleep 2
fi
...
sleep 1
res1 "$dn1"
# Ambil domain hasil dari pointing.sh
dn1full=$(cat /root/domain 2>/dev/null)
echo "$dn1full" > /root/scdomain
echo "$dn1full" > /etc/xray/scdomain
echo "$dn1full" > /etc/v2ray/scdomain
echo "$dn1full" > /etc/v2ray/domain
echo "IP=$dn1full" > /var/lib/ipvps.conf
print_success "Domain: ${dn1full}"
echo ""
clear
rm /root/subdomainx 2>/dev/null
fi
}
...
# Ambil expired date dari REPO peyxdev/esce melalui file ip atau ipx
if [[ -f /etc/xray/expired_date ]]; then
    IZIN=$(cat /etc/xray/expired_date)
else
    # Coba ambil dari repo peyxdev via raw github
